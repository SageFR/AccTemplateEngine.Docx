...
namespace TemplateEngine.Docx.Processors
{
    internal class ContentProcessor
    {
        ...
        private static Dictionary<XElement, List<XElement>> contentCache = new Dictionary<XElement, List<XElement>>();
        private static Dictionary<XElement, string> tagNameCache = new Dictionary<XElement, string>();
        private IEnumerable<XElement> FindContentControls(XElement content, string tagName)
        {
            if (!contentCache.ContainsKey(content))
            {
                var elements = content.FirstLevelDescendantsAndSelf(W.sdt).ToList();
                contentCache[content] = elements;
                foreach (var elt in elements)
                {
                    tagNameCache[elt] = elt.SdtTagName();
                }
            }
            return contentCache[content].Where(sdt => tagName == tagNameCache[sdt]);
            return content
        ...
        {
            contentCache = new Dictionary<XElement, List<XElement>>();
            tagNameCache = new Dictionary<XElement, string>();
        }
    }
}
